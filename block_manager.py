...
class BlockManager:
    ...
    def __init__(
        ...
        # Pre-allocate KV cache pool: (num_blocks, num_layers, 2, num_heads, head_dim, block_size)
        # 2 is for K and V caches
        self.kv_cache_pool = torch.zeros(
            (num_blocks, num_layers, 2, num_heads, head_dim, block_size),
            dtype=dtype,
            device=device
        )
